...
export function ProductCard({ product, onClick, primaryColor = '#000', accentColor = '#fff' }: ProductCardProps) {
  return (
    <motion.div
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {product.tags?.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 text-xs font-medium rounded-full bg-orange-50 text-orange-600"
              >
                {tag}
              </span>
            ))}
